            ...
            <div class="row">
                ...
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-user">
                          ...
                          <li><a href="<?php echo base_url()?>confirmation/NonIDFIndents"> New Non IDF Indents</a>
                        </li>
...
<div class="panel-body">												
	...
	<p></p>					<div class="dataTable_wrapper" style="width:700px;height:300px" id="pendingInvoices">	
...
<tbody>
	...
	{
		$TOTAL = $TOTAL + $key1->TotalValue;
	}
	 if(isset($ShowAllNewIndentData)){
		
		 foreach($ShowAllNewIndentData as $key1);
		 $IndentNo = $key1->IndentNo;
		 $ProfInvoiceNo = $key1->ProfInvoiceNo;
		 $ProfInvoiceDate = $key1->ProfInvoiceDate;
		 $Currency = $key1->Currency;
		 ?>
		 
	 <tr <?php if($IndentNo==$key1->IndentNo){echo "style='background-color:#990033;color:#FFF'";}?>>
		...
		// print_r($AllPendingIDFData);} else echo "checked";
		 ?>
	<?php 
	if(isset($ShowAllNewIndent)){
	foreach($ShowAllNewIndent as $key1):
	
	if($this->confirmation_model-> ShowCommodityUnderIndent($key1->IndentNo)!=NULL)	
	{
	?>

	 <tr >
		<td ><input type="radio" <?php if($IndentNo==$key1->IndentNo){echo "checked";}?> name="invoicedaya" onchange="loadIDFNumber('<?php echo $key1->IndentNo?>')"></td>
			<td style="cell-padding:55px"><?php echo $key1->IndentNo?></td>
...
endforeach;
	...
	<form role="form" id="Amendments" action="confirmation/safeConfirmation">
...
<select class="form-control select2" style="width:95%"  name="Bank" id="indentNo">
	...
	<input type="hidden" value="<?php echo $IndentNumber?>" id="IndentNumber">
<textarea class="form-control"   name="Message" rows="5"  style="width:95%;font-size: 9pt">
As per your Pro forma invoice nos. <?php echo $ProfInvoiceNo; ?> dated <?php echo  substr($ProfInvoiceDate,0,10); ?> for <?php echo $Currency." ".number_format($TOTAL, 2, '.', ',');?>  ExWorks.
For Immediate FCL shipment house through bill of Landing stating the final destination as ICD Embakasi, Nairobi
N/B:
This Indent is subject to Pre-shipment inspection by either SGC, Intertek or Bureau Veritus who must issue a Certificate of Conformity before the goods are Shipped.
<?php 

//if(isset($FindConfirmMessage)){echo $FindConfirmMessage;
//}
?>
					...
					if($this->Reports_model->confirmationDetails($IndentNumber)!=NULL)
					{
						?>
						<a  id="" class="btn btn-primary btn-sm pull-right" href="<?php echo base_url()?>Reports/SupplierCopy/<?php echo $IndentNumber?>" target="blank">Print Preview</a>
					<?PHP
					}
					?>
							</form>
					...
					<div class="row">
                     ...
                     <div class="panel-body">
			 <div class="panel panel-default">		 
<div align="center"><label> Select Products  for Confirmation</label></DIV>
     <div class="table-responsive table-bordered" style="" >
...
<tbody>
	...
	foreach($ShowCommodityUnderIndent as $key1)
	{
		$SQL = "SELECT * FROM PurchaseOrderData WHERE IndentNo='{$IndentNo}' AND ProductCode='{$key1->ProductCode}'";
	...
	?>
	 <tr>
				...
				<td><?php echo number_format($key1->TotalValue, 2, '.', ',')?></td>
